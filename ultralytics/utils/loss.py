...
from ultralytics.utils.ops import crop_mask, crop_mask_obb_seg, xywh2xyxy, xyxy2xywh, xywhr2xyxyxyxy
...
class v8ObbSegLoss(v8DetectionLoss):
    """Criterion class for computing training losses."""
    ...
    def preprocess_obb_seg(self, targets, batch_size, scale_tensor):
        ...
        return out
    def bbox_decode_obb_seg(self, anchor_points, pred_dist, pred_angle):
        """
        Decode predicted object bounding box coordinates from anchor points and distribution.
    ...
    def __call__(self, preds, batch):
        ...
        try:
            ...
            # 这里用的obb_seg的preprocess
            targets = self.preprocess_obb_seg(targets.to(self.device), batch_size, scale_tensor=imgsz[[1, 0, 1, 0]])
        ...
        # Pboxes
        pred_bboxes_obb_seg = self.bbox_decode_obb_seg(anchor_points, pred_distri, pred_angle)
        bboxes_for_assigner = pred_bboxes_obb_seg.clone().detach()
        ...
        if fg_mask.sum():
            ...
            # 还原target_bboxes,以供后面计算分割的loss使用
            target_bboxes[..., :4] *= stride_tensor
        ...
        # WARNING: lines below prevent Multi-GPU DDP 'unused gradient' PyTorch errors, do not remove
            ...
            loss[1] += (proto * 0).sum() + (pred_masks * 0).sum() + (pred_angle * 0).sum() # inf sums may lead to nan loss

        # # Bbox loss for obb
        # if fg_mask.sum():
        #     target_bboxes[..., :4] /= stride_tensor
        #     loss[0], loss[2] = self.bbox_loss(
        #         pred_distri, pred_bboxes_obb_seg, anchor_points, target_bboxes, target_scores, target_scores_sum, fg_mask
        #     )
        # else:
    ...
    def calculate_segmentation_loss_obb_seg(
        self,
        fg_mask: torch.Tensor,
        masks: torch.Tensor,
        target_gt_idx: torch.Tensor,

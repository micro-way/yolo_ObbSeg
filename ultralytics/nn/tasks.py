    ...
    def __init__(self, cfg="yolov8n.yaml", ch=3, nc=None, verbose=True):  # model, input channels, number of classes
        ...
        if isinstance(m, Detect):  # includes all Detect subclasses like Segment, Pose, OBB, WorldDetect
            ...
            def _forward(x):
                ...
                return self.forward(x)[0] if isinstance(m, (Segment, Pose, OBB, ObbSegHead)) else self.forward(x)
...
def parse_model(d, ch, verbose=True):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(d["backbone"] + d["head"]):  # from, number, module, args
        ...
        elif m in {Detect, WorldDetect, Segment, Pose, OBB, ImagePoolingAttn, v10Detect, ObbSegHead}:
            args.append([ch[x] for x in f])
            if m is Segment:
                args[2] = make_divisible(min(args[2], max_channels) * width, 8)
            if m is ObbSegHead:
                args[2] = make_divisible(min(args[2], max_channels) * width, 8)
            if m in {Detect, Segment, Pose, OBB, ObbSegHead}:
                m.legacy = legacy
...
def guess_model_task(model):
    ...
    if isinstance(model, (str, Path)):
        ...
        if "-seg" in model.stem or "segment" in model.parts:
            ...
            return "pose"
        elif "-obb_seg" in model.stem or "obb_seg" in model.parts:
            return "obb_seg"
